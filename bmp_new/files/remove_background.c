...
int main()
{
    BMP_Image *testImage = BMP_Open("test.bmp");
    BMP_Image *originalImage = BMP_Open("original.bmp");
    if(testImage == NULL)
    {
    ...
    BMP_ForEachPixel(testImage, pixfunc_compute_min_max);
    ...
    BMP_Free(testImage);
    BMP_Save(originalImage, "original_modified.bmp");
